...
| **再導出** | 同じ情報を調べ直した、または同じ訂正を受けた | 2 回目で候補になる。1 回目は台帳に残るだけ |
| **明示** | User が記録を明示的に指示した | 即時（経路は 7 節） |
...
**照合は pg_trgm で行う。** 埋め込みモデルは使わない。`pain_report`（8 節）は登録時に prevention の類似エントリと、退役済み Memory の tombstone を照合して返す。

- 類似の `friction` が既にある場合、二度目が成立し、両エントリを根拠とする昇格候補を自動生成する
...
読む経路はすべて push である。検索ツールは持たない。push には三つの経路があり、受け持つ失敗が異なる。
...
| `pain_report` | 痛みを台帳へ記録し、類似エントリと tombstone を返す。二度目・incident なら候補を生成 |
| `memory_list` | 指定 Scope の active な記憶を列挙する。push 対象外の Scope を明示的に見にいく唯一の読み口 |
| `temporary_put` | 期限つき条件の記録 |

v1 の 9 ツールから半減以下になる。`memory_search` / `memory_get` / `entity_resolve` / `memory_propose` / `scratch_put` / `scratch_get` は、対応する機構ごと廃止する。
...
| 三層 Retrieval / `memory_search` | 全量 push に置き換え |
...
| Scratch / `context_put` の scratch 側 | 抽出の入力だった。抽出が無い |
...
- 二度目の照合が実際には拾えていない（台帳に類似エントリがあるのに気づかれない）。pg_trgm の閾値か prevention の書き方を較正する
