# Mashu — Shared Agent Memory Layer / MVP 実装仕様書 v0.16
...
- v0.16: 読む側の保証を、セッションではなく行為に置く(30.1節)。行為の門を実装し、採用済みの知識を行為の直前に突きつけて一度拒否する。計器の嘘を直す。開始時の計数が JSON の足場を数えず、上限が実費の半分に対して掛かっていた(21.2節)。正しく数えたところ上限を超えたので、押し込む行から何も言っていない欄を落とす(21.2節)。Layer 1 に token 上限を入れる。件数だけが上限で本文長が無制限だった(21.1節)。日次予算をすべての抽出経路に掛ける(16.3節)。完了した版を抽出の入力集合から外す(16.1節)。捕捉の走査を固定窓から watermark へ替え、走査の沈黙を health に載せる(16.3節)。Review 随意と矛盾していた待ち行列の警告を直す(17節)。検査ファイルの欠落が黙って通る経路を塞ぐ(24節)。事故分類に `source_preemption` を足す(27.5節)。Scope の現在状態の書き手を抽出へ置く(14節)
- v0.15: 承認を通った知識が腐る経路を塞ぐ。採用済みの Memory に点検期日を入れ、期日を過ぎたものを掃き出しへ載せる(13.2節)。期日への回答は原則として Agent が行い、人へ届くのは誰も何も言えなかったものだけにする。ただし Agent の確認が一つの Memory を人から隠せる上限を設ける(13.2・30節 段 B)。採用済みどうしの類似対を掃き出しに載せ、畳むか「二つである」と記録するかを選べるようにする(20.3節)。点検待ちの件数を Session Bootstrap に載せる(21.2節)。未審査比率が却下・棚上げを未決定として数えていた誤りを直す(27.3節)
- v0.14: Review の扱いを、単位ではなく操作の側から直す。束を題名の目次として開き、矢印で読み、キー 1 打で決める形にする。決定はその場で 1 件ずつ確定し、中断しても手前が残る(18.1節)。--all が束そのものを広げていなかった経路を直す(18.1節)。待っている束の一覧を最上位に置き、画面に収まる分だけ描く。根拠の表示を status 名から「その根拠がこの判断にとって何を意味するか」へ改める(19節)。Review の頁に同じ Scope の近い Entity を併記する(20節)。エスケープ列を終端まで読む。上限超過時の短形をプロンプトで受け、エディタは選べるものを選ぶ(18.1・21.2節)。寿命を持つ採用済み知識の掃き出しを実装(13.1・30節 段 B)。完了した版が現役の顔で返る経路を塞ぐ(21.1節)。掃き出しに「まだ真、後で聞く」を足す。退役だけを出口にすると、退役でない判断が退役として記録される(30節 段 B)。却下済み提案が User 自身の退役を塞ぐ経路を断つ(15.1節)
...
`context_put` は Agent が期限つきの条件を書く経路である。window は 14 日を上限とし、それより長い主張は「あり方についての主張が期限を着ているだけ」なので `memory_propose` へ回す。**書いたものは Retrieval に載るが Bootstrap には載らない。** 押し込む権限と書ける権限を分けるためで、Bootstrap は頼んでいないセッションにも届く以上、誤った window の費用が検索結果の場合と桁で違う(25.2節)。
...
13.1 節は寿命で三分し、indefinite なものだけを Memory に置いた。実データで測ると、この三分は**書く時点の分類としては正しく、置いたあとの扱いとしては足りない**ことが分かった。

Task と State は自分で終わりを抱えているので、掃き出しは type だけで拾える。問題は Fact・Observation・Interpretation で、これらは indefinite として置かれながら実際には黙って古くなる。「採用している版」「ある条件で観測した挙動」「なぜそう動いたかの解釈」は、日付を名乗らないまま真でなくなる。実在庫 226 件のうち 46 件がこの状態にあり、寿命の読みが一つも当たらなかった。

そこで **type ごとに「誰も見ないまま置いておく期間」**を定める。期間を過ぎたものは掃き出しに載る。

| type | 期間 | 読み |
|---|---|---|
| task, state | 0 日 | 型が終わりを抱えている。従来どおり常に載る |
| hypothesis, interpretation | 60 日 | 見立てであることが型に書いてある |
| observation, fact | 180 日 | 観測・確定値。黙って古くなる代表 |
| decision, preference | 365 日 | 覆るのは稀だが、覆らないとは言えない |

これは**判断ではなく時計**である。載ったことが言うのは「誰も見ていない」であって、「誤っている」ではない。この区別は掃き出しの文面が担う。

**期日に答えるのは原則として人ではない。** 抽出は既にその Scope の Active 全文を読んでいるので、期日を過ぎたものに印を付ければ、セッション中にその内容が現に成り立っていた裏付けを見た Agent がそのまま確認を記録できる(16.1節)。人へ届くのは、どのセッションも何も言えなかったものだけになる。

確認は Proposal を経ずに直接書かれる。Version も status も動かず、変わるのは「次に人へ聞く日」だけだからである。ただしそれは信頼ではなく上限で担保する。**Agent の確認が続いても、最後に人が何かを決めた日から 365 日を超えて期日を延ばすことはできない。** 根拠の弱い確認が毎晩繰り返されると、その Memory は誰にも届かないまま配られ続ける。上限があれば、確認が過剰な場合に買えるのは一年であって永遠ではない。

非対称は退役と逆向きにする。退役は「迷ったら出す」であり、確認は「迷ったら出さない」である。出し損ねた場合の損は人が一度読むことで済み、根拠なく出した場合の損はその Memory が期日まで人の目から消えることだからである。

## 14. Current State

**v0.16 の追加**: 現在状態を書く担い手を抽出へ置く。

形は最初から正しかった。Scope につき原則 1 つの Version 管理された文書で、21.2 節はそれを検索を経ずに押し込める。にもかかわらず実測では、採用済み 226 件・Scope 8 個に対して**立っている現在状態は 1 件**だった。抽出は `state` を 8 つの type の一つとして並べるだけで、**書き換える責務をどこにも置いていなかった**。抽出が出すのは個別項目が最も抵抗の低い出力形であり、結果として 138 件がすべて検索の山へ積まれた。

構造を作り直しても、書き手が居なければ同じ空の区画がもう一度できる。これは推測ではない。掃き出しの機構は先に存在し、誰も引っ張らなかったので一度も起動されなかった(30節 段 B の実装コメントが記録している)。

したがって抽出の出力に「その Scope の現在状態の全文」を加える。これが Memory の量を増やさない理由は三つある。**積まずに置き換える**ので在庫が増えない。上限は削って守るのではなく**拒否する型**である(末尾を落とすと「全体に見えるが全体ではない要約」ができ、そのほうが害が大きい)。state へ吸われるぶん個別 Proposal は減る方向に働く。**三つ目は期待であって保証ではないので、実装後に追加提案の件数で確かめる。**

着地は candidate である(17節)。直したのは「誰も書いていない」ことであって「人が邪魔をしている」ことではなく、Review は実測で追いついている。
...
**User 自身の退役は却下に妨げられない**(v0.14):

同じ理由が人にも掛かる。Agent の退役提案が却下されたあと、その Task が実際に終わっても、重複チェックは人が `mashu retire` で終わりを述べる道まで塞いでいた。**却下されたのは Agent の提案であって、いま終わっているかどうかについての裁定ではない。**16.1節は User 発話それ自体を退役の契機と定めているのだから、この経路は通す。

したがって、待機中の Proposal があればそれを承認し(これが本来の合流)、すべて決着済みであれば**以前の裁定を添えて新しい Proposal を出す**。人は自分が何を踏み越えたかを画面で見る。

実測では、掃き出しに立っていた 14 件のうち 3 件がこの状態にあり、いずれも `mashu-worker` の completed 提案が却下されたまま残っていた。「なぜ一部だけ退役できないのか」は画面からは分からず、拒否の文面も呼び出し側で握り潰されていた。**拒否は値として返し、印字する側が置き場所を決める。**
## 16. Write Policy
...
出所を問わず、本文・title・directive に個人の識別情報(本名、所属、学籍番号、**ホームディレクトリを含む絶対パス**)を含む提案は、Gate の手前で拒否する。書き換えではなく拒否とする。入り口で本文を編集すれば「誰も選んでいない本文の Version」が残り、この層が守っている provenance そのものが壊れる。

Review はこれを捕まえない。Review が読んでいるのは**主張が正しいか**であって、パスの途中にユーザー名が綴られているかではない。そして知識状態は毎セッション読み出され、そこから Agent の生成物へ流れていくので、ここに入った識別情報は「漏れなかった」のではなく**ゆっくり公開された**ことになる。

パターンの一覧はリポジトリの外に置く(commit hook が読むのと同じファイル)。禁じるものの一覧は、それ自体が禁じるものの一覧だからである。一覧が見つからないときは「合格」ではなく**「検査できなかった」**と報告する。ここで黙ると、走って通ったのと区別がつかない。
...
そして **health は人間の巡回ではなく、必ず読まれる経路、すなわち session_bootstrap へ押し込む。** 失敗件数と最古 queued age が閾値を超えたら、次のセッションの Bootstrap が警告を運ぶ。**誰も面倒を見ない期間の監視者は、次に来るセッションである。**

**v0.15**: 同じ経路に載る待ち行列は三つになる。捕捉が止まっていれば新しいものが入ってこない。Review が滞っていれば入ったものの確度が上がらない。**点検が滞っていれば、既に入っているものが古いまま配られる。** 三つ目だけが Bootstrap に載っておらず、その結果として掃き出しは実装されたまま一度も起動されず、在庫が壊れているのを人が偶然見つけるまで放置された。載せるのは件数と最古の超過日数だけで、対の検出は載せない。対の検出は Scope ごとに在庫を走査するので、**全セッションが払う固定費として置けるコストではない**(21.2節)。
...
**worker が出せる操作は三つ**: 新規作成、**既存 Entity への更新(新しい Version の追加)**、退役提案。更新が要るのは、それが無いと訂正の手段が消えるためである。同じ概念について別 title で新規を出せば知識状態が割れ、似た title で出せば人が突き合わせるまで宙に浮く。32節が挙げる「記録・**訂正**・保留・棄却・再評価」のうち訂正だけが、毎晩走る唯一の書き手から欠けている状態になる。

**却下は「時効のある禁止」であって「時効のある事実」ではない。** 15.1節の重複チェックは、既に裁定されたものを見て判断できる提案者を前提にしている。worker は見られないので、この検査は worker にとって恒久かつ無言の禁止になる。却下された title 一つがその Scope の近傍すべてを永久に抑圧し、「まだ早い」で退けた退役提案は、その Task が実際に終わっても二度と出せない。したがって一定期間(実装では 30 日)を過ぎた却下は禁止としては失効し、worker は再提案できる。ただし**以前の裁定を新しい Proposal に載せて出す**ので、Review が出会うのは繰り返しではなく反論である。pending の Proposal がある間は何も解除しない。

**モデル呼び出しはトランザクションの外に置く。** 呼び出しは 10 分かかりうる。その間トランザクションを開いたままにすると claim が取った行ロックを保持しつづけ、走行中のセッションが自分の Scratch を書くのを塞ぎ、管理された PostgreSQL では idle-in-transaction のタイムアウトで殺される。**モデル側の失敗と見分けがつかない死に方**である。読む・呼ぶ・書くを三つのトランザクションに分け、その隙間を守るのは claim 自身とする。

**claim には claim した時刻を持たせる。** 死んだ worker が握ったままの run を回収する条件を「作成時刻から一定時間」で書くと、sweeper の滞留分と、日次予算で 24 時間 defer された分は**取られた瞬間に「古い」**ことになり、一つ目の worker がモデルを呼んでいる最中に二つ目が同じ transcript を同じ checkpoint から読む。行ロックはこの境界を越えない(claim は仕事の開始前に commit する。それが 10 分かけられる理由でもある)。

**転写ログは untrusted としてタグで囲って渡す。** 会話ログにはそのセッションが読んだもの(Web ページ、ファイル、他人の文書)がそのまま含まれ、その中の見出しはこちらの見出しと区別がつかない。被害半径は限られている(退役は見せた id に固定され、他はすべて candidate に落ちる)が、Proposal の**本文**は Layer 2 に載り、後続の Agent には「使ってよい」と伝えられる。閉じタグの偽装を防ぐため、ログ中の閉じタグは無害化する。
...
三区分は変えない。撤回するのは、その周りに暗黙のまま残っていた運用概念、すなわち**「candidate はいつか全件 Review されて確定する」**のほうである。
...
単位を束にしても、束を全文印字してから 1 行の指示を打たせる形では負荷が残る。22 件の束は 299 行あり、指示を組み立てる時点で本文は画面から流れ去っている。**文脈スイッチは 1 回に減っても、番号と項目の対応を記憶に置く負荷が新しく生まれていた。**単位は正しく、扱いが誤っていた。

そこで束はまず**題名だけの目次**として出す。ここで 1 打で束ごと通せる形が、上の「束で通す」を実際の既定にする。読む場合は矢印で項目を送り、1 打で決める。理由が必須の操作だけ、その場で 1 行を尋ねる。

**束そのものも選べる必要がある。**どの束が最古かはセッションが終わった時刻の偶然でしかなく、10 分しかない読み手がその 10 分を 22 件の束に使わされる理由はない。したがって最上位に**待っている束の一覧**を置き、Scope、件数、滞留日数を並べて、どれからでも開けるようにする。

**画面に入る分しか出さない。**溢れた画面は読み手に見出しまでスクロールで戻らせる。一覧は端末の高さに合わせて窓を切り、残りは件数で言う。本文が長い項目は画面ぶんずつ送る。折り返し幅も端末に合わせる。

各決定は**その場で 1 トランザクションずつ確定**する。途中で抜けても手前は残り、次の `review` は残りから開く。Review が任意である以上、中断が損になる設計は採れない。

競合対象は他の Agent CLI が備える対話的な記憶編集である。**キー 1 打で決まらない Review は、その水準に対して選ばれない。**
...
Review で根拠を出す目的は、その Proposal を承認してよいかを決めることにある。したがって出すのは status の名前ではなく、**その根拠の立ち位置が、いま決めようとしている判断にとって何を意味するか**である。

採用は active_version のポインタで表すため(10節)、退役すればポインタは外れる。**active_version が NULL であることは「一度も採用されていない」と「採用されたのち誤りと判明した」の両方を覆う**が、この上に積んでよいかを決める読み手にとって両者は正反対である。したがって latest_version の status も併せて引き、どちらであったかを言う。

各根拠には `mashu inspect <id>` を添える。id だけを裸で置いても、それで何ができるかは書いていなければ伝わらない。
...
上の検査は Proposal を書く時点で走り、Review の時点ではその結果が残っていない。読み手は Proposal を一件ずつ単独で示され、**同じ主張がもう一度来ていることに気づくには、それまでの二百件を記憶しているしかない**。したがって同じ Scope 内の近い Entity を、その Proposal の頁に併記する。

判定は**題名が一致するか**と**中心化した cosine が閾値を超えるか**の二つに分ける。題名の一致は測定ではなく等値なので、数値を経由しない。実際、store に存在した題名完全一致 4 組はすべて同一の出どころを二度取り込んだもので、この経路がいちばん捕まえるべき事例だった。

中心化を挟むのは、この埋め込みの生 cosine が使えないためである。無関係な題名どうしが 0.82 付近に居り、無関係な Proposal の生の最近傍が 0.898 に達した。平均題名ベクトルを引いてから比べると分離し、実測時に待っていた 42 件のうち隣人が付いたのは 6 件、最大 2 件ずつだった。閾値 0.45 はここから採る。

**表示するだけで、自動では何もしない。**20 節が Entity 作成を自動化しないと定めているのと同じ理由による。
...
### 20.3 採用済みどうしの重複(v0.15)

20節の類似チェックは Proposal を書く時点で走り、**Review の時点では既に使い切られている。** Review は Proposal を 1 件ずつ開くので、既存の 200 件と突き合わせる作業は読み手の記憶に委ねられている。そこを通り抜けたものは採用され、以後どこからも測られない。実在庫では、ある Scope の 77 件のうち 10 件が採用済みの相手を隣に持っていた。

そこで**採用済みどうしの類似対**を掃き出しに載せる。対に対する答えは二つで、これらは程度の差ではない。

- **畳む**: もともと一つの概念だった。20.2 節の Merge を実行する
- **二つである**: 別の概念だった。何も変えないが、**その判定を記録する**

後者の記録が要る理由は、記録しないと同じ対が次も同じ点数で戻ってくるためである。終われない画面は、読み手に読み飛ばす習慣を教える。

対は片方向で一度だけ提示する。決めるのは二つの関係であって、どちらを先に見たかは判断の一部ではない。
...
Query
  ↓
Scope Detection(Scope が保持している Memory と照合。27.2節)
  ↓
Type Filter
  ↓
...
**採用済みの版が自分で「終わった」と言っている場合(v0.14)。**completed は active ポインタを外さない。「これは終わった」は探して見つける価値のある知識であり、同じことをもう一度尋ねられたときの答えだからである。しかしそれは**まだ終わっていない仕事の顔で届いてはならない**。この印が無い間、completed へ落とした Task は Layer 1 の先頭に本文ごと、何の注記も無く返っていた。**store は完了を保持し、version の reason にそう書き、読み手にはその逆を渡していた。**したがって version の status が candidate でないとき、Layer 1 は `finished` のタグと version の reason を載せる。提案中の退役より先に読む。決着しているものは、まだ誰も問うていない。
...
Layer 1 は 21節のパイプラインをそのまま通す。Layer 2 は本文を渡すため、Layer 1 と同じく content_embedding で引く。Layer 3 は、着地点の Scope の範囲内で title_embedding により引く。content を渡さない層に対して本文の類似度で順位をつける意味がないためである。

Layer 2 と Layer 3 に含めた Memory ID も、Layer 1 と同様に event_log の context_assembled へ記録する(22節)。

MVP の既定:

Layer 1 は常に返す。Layer 2 と Layer 3 は、クエリが着地した Scope に限る。Scope 全体を返す形は、Layer 3 が肥大したときに Context を圧迫するため MVP では採らない。

**着地点の定義(v0.12)**

Scope Detection が絞り込みを返したときはそれが着地点である。辞退したときの着地点は、**Layer 1 が当たった Entity の Scope ではなく、probe の上位一致が住んでいる Scope** とする。

両者は一点で食い違い、その一点が問題だった。Layer 1 は active_version しか読まないので、**採用済みを 1 件も持たない Scope は Layer 1 の行に決して現れない**。着地点をそこから取ると、その Scope の候補はどのクエリからも到達不能になる。21.1 が撤回した「Review が使用の門になる」構造が、絞り込みの副作用として復活していた。probe は Layer 1 と Layer 2 が渡せるもの両方を読むので、この食い違いを持たない。

probe が空、すなわちクエリが在庫のどれにも当たらなかったときに限り、Layer 1 の Scope へ落とす。
...
計器が JSON の足場を数えるようになったところ、開始時の pack は上限を超えていた。内訳を測ると、超過は規律の量ではなく**足場そのもの**であった。
...
行の足場            1,260
封筒                  190
----                2,772   本文を全件落としてもなお超過
```

1 行の実測は、言葉 100 に対して足場 96 である。13 個のキーのうち UUID が 3 つ、pack の全行で同値の欄が 2 つ(`type` / `delivery`)、null が 5 つを占めていた。

上限は請求書ではなく **admission control** であるから、足場は開始を高くするだけでは済まない。**どの常設の規則をセッションに伝えるかを決める予算を、何も言っていないキーが食う。**現に `would_fit` はこの時点で全件を拒否しており、その拒否理由は昇格しようとしている当の知識とは無関係であった。

そこで押し込む行は**何かを言っているキーだけ**を載せる。

| 欄 | 載せる条件 |
|---|---|
| `memory_id` | 常に。全文を取りに行く先 |
| `content` | 常に。`coalesce(directive, content)` |
| `title` | **本文が削られたときだけ** |
| `shortened` | 短形であるときだけ |
| 変更中の注記 | 保留中の提案があるときだけ。対で載せる |

`scope_id` / `type` / `delivery` / `version_id` は落とす。注記を対で載せるのは、片方だけでは理由の付かない主張になるためである。

最も効くのは**題と本文を両方送らないこと**である。directive は「人が Review した規則の短い形」で、題は名前である。ところが実際に書かれた題は完全文であり、両者は同じことを言っていた。本文が残る行では題を落とし、本文が削られた行でだけ題を残す。押し込みにおける題の役目は、消えた本文の身代わりだからである。

結果(実データ、実際に載る JSON での実測):
...
全体                  3,273   1,516
本文が削られた件数     14 件     0 件
would_fit             False    True(最も重い開始で 1,794)
```

足場の定数も実測に合わせ直した(行 90 → 22、索引 40 → 20、封筒 190 → 215)。索引の 40 は実測の 2 倍であった。安全側の誤りではあるが、**計器が嘘をつかない**という v0.16 の主眼と矛盾している。
...
**Scope の名前と一行要約にクエリを当てる方式そのものが、この規模では成立していない。** 閾値を下げても誤った Scope に絞り込むだけである。三語のラベルは、その Scope が何についてのものかを運べない。

**作り直し(2026-08-24、同じクエリ集合と同じ在庫)**

Scope はラベルではなく**その Scope が保持している Memory** から判定する。クエリを Memory 本文の索引、すなわち Layer 1 が既に十分な精度で順位づけしている索引に当て、上位の一致がどの Scope に集中しているかを見る。**絶対類似度の問いを順位と集中の問いに変える**わけで、実測が示したのは、この埋め込みが答えられるのは後者だけだということだった。

集中は**素の割合ではなく lift** で測る。「その Scope が上位一致の何割を占めるか」÷「その Scope が在庫の何割を占めるか」である。

素の割合には**大きさへの不変性が無い**。しかも自動捕捉が入るとそれは理論上の話ではなくなる。route は実際に作業するディレクトリを指すので、ひとつの Scope が在庫の大半を持つようになり、以後は質量だけで全クエリを取る。比にすると問いが変わる。**この Scope は、その大きさから予測されるより良く答えているか。**極端側の壊れ方も正しい。在庫のほとんどを持つ Scope はまず比を超えられないので検出されないが、在庫の大半に等しい Scope へ絞り込んでも得るものは無い。lift が 1.2 であることは、**検出されうる Scope の大きさの上限が在庫の 83% であること**と同義である。

**probe が読む範囲と、票の重み(2026-08-24、在庫 242 件・18 クエリ)**

上の作り直しには二つの欠落があり、実運用で両方が現れた。

**① probe が採用済みしか読んでいなかった。** 候補しか持たない Scope は probe に現れず、Layer 1 にも現れず、したがって Layer 2 の絞り込み先にも現れない。実在の在庫で 4 Scope 中 1 つがその状態にあり、その 37 件は**どのクエリからも取り出せなかった**。probe は Layer 1 と Layer 2 が渡せるもの、すなわち採用済みの version と未審査の candidate の両方を読む。21.1 の「Review は品質を確定する門であって使用の門ではない」は、上限だけでなく**読む経路にも**適用されなければならない。

**② 上位 25 件を等しく 1 票ずつ数えていた。** 順位 25 位が順位 1 位と同じ重みを持つので、近い外れを大量に持つ Scope が、正解を 1・2 位に持つ Scope を票数で押し切る。実例として「独白で鍵になる物証を先に出さない」は上位 2 件が正しい Scope にあり、0.82〜0.84 に溜まった別 Scope の 13 件に負けていた。**票は最上位からの距離で減衰させる**(exp、目盛り 0.008)。

floor は票の選別から外し、**最上位の一致だけにかける門**にする。絶対類似度がここで答えられる唯一の問い、すなわち「このクエリは何かに当たったか」がその役割である。仕事はしていて、在庫に主題が無い 5 クエリのうち 3 つをこの門が退けた。

判定は「一つの Scope が減衰後の票の 3/5 以上を占め、かつ lift 1.2 以上」とする。3/5 を要求する結果として**検出される Scope は最大 1 つ**になる。二つの Scope に跨がる問いは、片方だけを残す絞り込みより全体検索のほうがよく答えられる。

| | 旧(採用済みのみ・floor で選別・等重み) | 新(渡せるもの全部・門・減衰票) |
|---|---|---|
| 正しい Scope へ絞り込み | 5 | **10** |
| 辞退(全体を検索) | 3 | 3 |
| **誤った Scope へ絞り込み** | **7** | **0** |
| 在庫に無い主題を退けた | 3 / 5 | **5 / 5** |

誤った絞り込みが答えを丸ごと隠す唯一の失敗で、辞退は順位が薄まるだけである。二つ以上の Scope へ絞り込んだ場合も、正解を含んでいても誤りに数える。片方を落とす絞り込みが答えを隠す点は変わらないためである。

**正解が二つある問いの扱い**

「測定を始める前に何を決めておくべきか」には正解が二つある。在庫はこの規律を、作業の規律 の一般則としても、enrai の実例「難易度測定前に行動分岐を明文化する」としても保持しており、後者のほうが語彙が近いので上位に来る。当初これを 作業の規律 のみが正解として記録し、誤った絞り込みに数えていたが、**在庫の状態を誤って記述していたのは測定の側だった**。

そこで期待値に `a|b` の形を認め、第二の正解に着地した場合は `hit*` として別に数える。上の表の「新」10 件のうち 1 件がこれである。**結果を見てからラベルを緩めるのは測定が落ちなくなる道筋そのもの**なので、折り畳まずに残す。

この二重登録自体は片付けない。一般則と競技固有の実例は別の知識であり、title の類似度も 0.864 で Entity Resolution の 0.90 に届かない(20節)。

**閾値がなぜ流れるのか(2026-08-24)**

27.2 の最初の実測が決めた floor 0.85、作り直しが決めた floor 0.82 は、どちらも在庫が増えると効かなくなった。理由は埋め込みの**異方性**である。

保存済み 242 本の平均ベクトルのノルムは **0.904** だった。全部が単位ベクトルなのだから、これは**どのベクトルもその長さの 9 割が共通の一方向**であることを意味する。無関係な文書どうしの cosine は平均 +0.817、sd 0.032 に収まる。

**このモデルが返す類似度は、共通成分にごく薄い残差を足したものである。**絶対値の閾値はその残差の中に置くほかなく、残差の分布は在庫の構成が変われば動く。したがって閾値の再調整は対症であって、**同じ流れが必ず再発する**。順位と集中で判定する設計を採るのはこのためである。

参考までに、平均ベクトルを引いて再正規化すると文書どうしの cosine は中央値 −0.025・sd 0.118 に広がり、上位 1 位と 10 位の差は 0.031 から 0.177(5.7 倍)になる。**順位そのものは変わらない**(上位 1 位の Scope の正誤は 11 問中 9 問で同一)。中心化が改善するのは順位ではなく**順位差の読み取りやすさ**である。ただし中心化した値は「何にも当たっていない」を判定できず(在庫に無い主題のクエリが中心化後に高い集中を示す例が出た)、生の cosine による門と併用しなければ使えない。MVP では中心化を採らず、平均ベクトルの保守が要らない減衰票で足りている。

`mashu admin thresholds` がこの測定を再実行する。**測定はこの機構を再実装せず detect_scopes を直接呼ぶ。**測定が規則の写しを持つと、片方を編集した瞬間から系を測らなくなり、この節が捕まえようとしている流れがまさにその形で起きる。
...
**v0.15**: 未審査は latest_version の status から読み、active_version が無いことから読まない。この二つは見た目が似ていて意味が逆で、**却下・棚上げされた Entity にポインタが無いのは、誰かが決めたからである。** これを未審査に数えていたため、待ち行列が空の Scope が「4 分の 1 が未審査」と表示されていた。しかも決定済みのものは二度と決定されないので、その表示は永久に下がらない。指標が動かなくなる誤りは、値が誤っている以上に悪い。
...
- 15.1節の重複チェックを仕様(title embedding 照合)へ揃える

段 A は実装済み。model を呼ぶ部分は差し替え可能にし、実装を二つ持つ。仕様の第一選択(専用予算の API)と、鍵を要さない fallback(対話 CLI の subprocess)である。後者は cwd を sweeper が歩かない場所に置き、hook を切って走らせる。そうしないと**毎晩の抽出が翌晩の入力になる**。

**MCP server は、自分が仕えている CLI セッションの id を引き受ける。** これが無いと、Agent が書いた Scratch は二度と見つからない。worker は transcript から得た (source_cli, external_session_id) で引くので、Agent 名しか持たない session 行には照合するものが無い。結果として捕捉パイプラインの第一段は死ぬ。Scratch は書かれ、読まれず、消されず、抽出は毎回 transcript だけの値段を払い、しかも同じ論理セッションに対して行が二つでき、対話中の Proposal と抽出の Proposal が別々の Review 束へ落ちる(18.1節が束にした理由が消える)。id は環境変数から取る(各 CLI が既に export している)。

**そして段 A には、それを呼ぶものが要る。** `mashu work` は一回走って終わる。定期実行のユニットが無ければ「人が打ったときだけ書き込まれる」ままで、較正点の「人手ゼロで生まれている」は原理的に満たせない。ユニットは同梱するが**読み込みは行わない。** 読み込むことは、毎晩誰も見ていないところでモデルの枠を使いはじめることであり、枠の持ち主の判断である。
...
0. **時計による点検**(v0.15)は退役ではなく、退役の必要を人へ届ける機構である。採用済みの Memory は type ごとの期間を過ぎると掃き出しに載り(13.2節)、期日への回答は原則として抽出が行う。ここが無いあいだ、退役の契機は「セッションの記録が既存の内容と矛盾したとき」だけであり、**黙って古くなったものには誰も出会わなかった**
1. **時限による退役**は Temporary Context の read filter。判断ゼロ、worker 不要
2. **User 発話による退役**は、完了・棄却の宣言を拾って Auto Commit で落とす。書いた本人の判断を時計と transcript が執行するだけで、新しい判断はない。人が直接叩く `mashu retire <id> completed|disproven|dormant --reason` を足す(16.1節が User Explicit を契機と定めながら、人間用の入口が無かった)
3. **Agent 推論による退役**は、**無人では落とさない。** candidate として積み、Layer 1 の当該項目に「退役候補あり(理由)」の注記を付けて返す(21.1節)

実装上、worker は退役提案に必ず `hold_for_review` を渡す。17節が Auto Commit に置いている「単純な Task 完了」は、人が「終わった」と言う場合のために書かれた行であり、worker の推論はそこに乗らない。`hold_for_review` は Gate を厳しくする方向にしか効かないので、Human Review Required の行はそのまま残る。

3 で「N 日誰も異議を挟まなければ自動で dormant へ落とす」を採らない理由: **それは「無反応」を「同意」と読む機構であり、無人の週にはまさに全員が無反応である。** 誤退役は誤追加より発見が遅い。消えたものは検索に現れない。

0 が同じ罠を踏まないのは、無反応の読み方が逆だからである。**無反応は「まだ真」ではなく「まだ人へ聞いていない」と読まれ、その項目は掃き出しに載ったままになる。** Agent が確認を書いたときだけ期日が動き、それも 13.2 節の上限までしか動かない。何も起きなければ、届く先は人である。
...
**実装(v0.13)**: `mashu trial --open / --close`。窓は event_log の trial_opened / trial_closed で表し、専用のテーブルを持たない。一度に一つで、開いて閉じるだけで編集されないものは、event_log が既にそういう帳面だからである。二つ目を開くことは拒む。重なった窓では集計がどちらのものか言えなくなり、**集計はこの作業の唯一の出力**である。

窓の中の事故は `mashu trial` が原因ごとに数える。窓を開かずに数えた件数は「いつからの件数か」を言えない。

**手順の側で押さえるもの**: 自動書き込みの停止だけでは切り替わらない。CLI の常設指示(`CLAUDE.md` / `AGENTS.md`)が古い記憶層を読めと書いていれば Agent はそこを読む。実測で確認した(自動書き込みを止めた状態でも、常設指示の側から memory 層が参照されうる)。手順は README の「切替」に置く。

事故の分類に**三つ目を足す**。旧二分類は Bootstrap の内容不足と pull の動機不足だったが、自動捕捉を入れると **捕捉漏れ、つまりそもそも書かれていなかった場合**が独立の原因になる。旧計画はこの分類を表現できなかった。
...
いずれも観測されなければ、この計画の中心仮説、すなわち**捕捉と退役の大半は判断ではなく寿命の型であり、型は無人で執行できる**は当面反証されない。

MVP 後: Codex / Gemini 接続の拡張、Web UI、Conflict の自動検出、worker の User 主張(16.3節の二重の絞りを満たしたうえで)。

### 30.1 読む側の保証へ(v0.16 の方針)

段 A から段 E までの実装は入り、書く側・審査側・採用済みの鮮度は動いている。実測で、直近 14 日に 326 件を 41 束で決裁、束あたりの滞留は中央値 4.8 時間、未審査比率はほぼ 0%、点検期日の残は 0。**動いていないのは読む側の保証だけである。**

#### 観測された事故

委譲先モデルを選ぶ場面で、Agent は `memory_search` を呼ばず、常設指示ファイルの既定値だけで決めて誤った宛先へ投げた。

- 正しい割り当ては採用済みの preference として蔵にあり、Layer 1 に類似度 0.847 で返る状態だった
- `session_bootstrap` は正常に発火し、返した Scope 索引の一行要約には当の語が含まれていた

6.1 節は「Agent は知らないことを知らないので呼ぶ動機を持てない」と書き、だから索引を押し込む設計にしている。この事故はそれではない。**答えを既に持っていると思っている状態**で、常に文脈にいて自分が古いとも範囲外とも申告しない静的ファイルに先取りされた。

事故のあとの対応も同じ轍を踏んだ。事故記録を書き、常設指示ファイルをもう一箇所書き換えた。同じ一つの規則について四つ目と五つ目の置き場所を作っただけで、どちらも次に読まれる確率を上げていない。**事故から Memory を書く反射そのものが、次の事故の材料を作る。**

#### 三点の裁定

外部レビュー二本と再設計案一本を突き合わせた結果、次を採る。

**一。第一因は注入の欠如ではなく、誤った可変値が常に注入されていたことである。** 常設指示ファイルは既に完璧に機能する push 経路であり、それが誤った答えを運んだ。門を建てても、静的ファイルが確信ありげな答えを持ち続ける限り競合は残る。**答えを持たない情報源には先取りされようがない。** よって最初の一手は追加ではなく削除で、常設指示ファイルからモデル割当などの可変値を落とし、規律と接続手順だけを残す。

**二。保証の単位はセッションではなく行為である。** 6.1 節の第二段(セッション開始フックからの注入)はこの事故を防がない。Bootstrap は発火していたからである。文脈に存在することと、その瞬間の判断材料として前景にあることは別で、長いセッションでは開始時の内容が後続の読み込みと推論の後ろへ沈む。要るのは、委譲・コミット・接続のような行為の直前に、その行為に必要な現在状態を突きつける小さな門である。

**Gateway(Mashu 自身がモデル呼び出しを所有する)は採らない。** 保証は最も強くなるが、日常の作業は既存の CLI で流れ続けるので、**保証が掛かる場所と時間の使われる場所が乖離する。** 使用時間の大半が compatibility path に落ちるなら、保証は空を守る。運用者 1 名に対して釣り合わない。正しい粒度は会話全体ではなく行為であり、それは PreToolUse フックと、両 CLI が共通に呼ぶラッパーの射程にある。この型は RCCS への接続で既に実証済みで、行為そのものを唯一の経路へ閉じることで解決した先例がある。

限界を明記する。この保証はツール呼び出しの形をとる判断にしか掛からない。文章だけで誤った結論に至る場合は覆えない。ただし記録された事故は委譲・接続・コミットに集中しており、当面はその境界で足りる。

**三。Evidence / Scope State / Archive への作り直しは採らない。** 三区画のうち二つは既にある(Scope State = 14 節 + 21.2 節の `scope_required`、Archive = Layer 3)。そして**空である。** 採用済み 226 件に対し、立っている Current State は 1 件、`scope_required` の押し込みは 4 件でうち 3 件は完了済み。8 つの Scope のうち 7 つが現在状態を持たない。

空である理由は三つで、どれもスキーマの問題ではない。抽出プロンプトが `state` を type の一つとして並べるだけで**編集の責務をどこにも置いていない**こと。昇格が人の判断と短形の記述を要するのに、そこへ引っ張るものが無いこと。既存の state が「維持される文書」ではなく「終われば退役するもの」として扱われていること。

**構造を作り直しても、書き手を置かなければ同じ空の区画をもう一度作る。** これは推測ではない。掃き出しの機構は先に存在し、誰も引っ張らなかったので一度も起動されなかった。同じ法則がここにも掛かる。

#### Scope State を小さく保つもの

21.2 節が上限を持つのは、最初の版が push 対象を type で決め、実測 45,840 token が 2,000 の上限に当たったからである。教訓の本質は「集合の固定費は在庫量の関数になる」であった。

**集合は足すことしかできないので増える。一つの文書は書き換えが置き換えなので、書き手に予算を課せば大きくなれない。** 14 節は最初から State を Scope ごとに原則 1 つの Version 管理された文書と定めており、形は正しい。欠けているのは書き手だけである。

担い手は抽出に置く。抽出は既に当該 Scope の Active 全文を読んでいるので、出力に state の書き換え案(全文、上限つき、超過は機械的に拒否)を加え、candidate として既存の Review へ落とす。これが Memory を増やす方向に働かない理由は三つある。積まずに置き換えるので在庫が増えない。上限は削って守るのではなく拒否する型である。state へ吸われるぶん、個別 Entity としての新規 Proposal は減る方向に働く。三つ目は期待であって保証ではないので、実装後に追加提案の件数で確かめる。

#### 行為の門(v0.16 実装)

`action_guard` は採用済みの Memory ひとつを、ひとつの行為の種類に留める。その行為が走る前に、留められたものが呼び出し側の前に置かれ、**呼び出しは一度拒否される。**

拒否が要点である。これは読めという注意書きではなく、**読まされずには進めない**という状態である。注意書きは従順さに依存し、今回の事故はまさに従順さの側で起きた。

行為の名前はツール名ではない。同じ判断は CLI ごとに別のツールで実行されるので、蔵がツール名で引かれると client ごとに行が要る。対応づけはフックの側、つまり端に置く。

留めるのは Entity であって Version ではない。本文を書き直せば門の言うことも変わり、誰もこの表を覚えていなくてよい。

**発火は 1 セッションにつき行為の種類ごとに一度。** 毎回発火する門は読まれない門であり、それは 16.3 節と 20.3 節が別々の画面について既に書いている失敗形である(常時赤い警告は読まれない。終われない画面は読み飛ばす習慣を教える)。同一セッションの二度目の委譲は、一度目の答えがまだ近くにある。

**発火はすべて記録する。** 半径が正しいかどうかは発火頻度で決まり、それは推論ではなく台帳が答える問いである。

門が動かないときは通す。蔵に届かないことは、いま下そうとしている判断についての証拠ではない。

#### 順序

1. **削除。** 常設指示ファイルから可変値を落とす。正は蔵の採用済み preference
2. **計器の嘘と予算の穴。** `_total_tokens` の過少報告(実ペイロード 3,208 token を 1,549 と報告する計器は、拒否型 admission control という不変条件そのものを無効にする)、codex 経路の日次予算が無限であること、完了した版が抽出用の Active 集合に残り続けること、Layer 1 に token 上限が無いこと、`upkeep` が task/state の 0 日により原理的に常時赤くなること、`backlog` の説明と SQL の齟齬、PII 検査のファイル欠落が黙って通ること
3. **行為の門を委譲だけに絞って入れる。** 宛先が蔵の採用済み preference と食い違えば拒否して理由を返す。読ませる保証ではなく、読まずに進めない保証
4. **捕捉の走査を固定日数から watermark へ替え、最後に走査した時刻を health に載せる。** 一年無人で沈黙する唯一の穴がここである。transcript はディスクに残るので、それ自体が spool であり、別の追記領域は要らない
5. **Scope State の書き手を抽出へ置く**(上の節)
6. **事故分類に `source_preemption` を足す。** 今回の事故は現行の分類のどれでもない。数えられない事故は設計へ戻らない。既存の記録は `pull` として入っているが、これは誤った分類である

**六つとも v0.16 で実装済み。** 5 の三つ目の理由(個別 Proposal が減る)だけが未検証で、追加提案の件数で確かめる。

#### 作らないもの

Gateway。三区画への移行、スキーマ変更、改名。Layer 2 の加齢と cold 化(21.1 節が実測なしに設計しないと定めており、未審査比率はほぼ 0% で前提が現に起きていない。ただしこれは現在の事実であって恒久的な性質ではなく、Review が長く止まれば前提は反転する)。Bootstrap への policy manifest(exact lookup を Agent が行うことに依存するので、従順さ依存が一段ずれるだけ)。Layer 3 の全 Scope probe(穴は実在するが未観測)。自動確認の構造化根拠。actor の principal 化(1 名の運用に認証基盤は過重であり、**actor は監査ラベルであって principal ではない**と明記して受ける)。

そして**事故対応の反射を一つ消す。** 「事故が起きた、それを防ぐ Memory を書く」をやめ、次の順に落とす。削除できる静的値は無いか。invariant・validator・transaction にできるか。現在状態の問題なら Scope State を書き換える。過去の証拠として意味があるなら履歴に残す。それでも常時参照する知識として要るものだけを Memory にする。
