...
Layer 2 は本文を渡す層であり、件数が増えれば Layer 1 と Token を奪い合う。上限を二つ設ける。

- **相対上限**: 単一 Retrieval 内で、Layer 2 の件数は Layer 1 の件数を超えない
- **絶対上限**: Layer 2 の合計を 1500 token とする

順位は Layer 1 と同じ類似度順とし、上限を超えた分は落とす。

相対上限を置く理由は Token 予算ではなく、`unreviewed` タグの識別力にある。

タグが Agent の判断を変えられるのは、タグの付いていない情報が併存している場合に限られる。渡した内容の大半が未審査になると、Agent はタグを無視して未審査の内容を知識として扱うか、タグに従って何も確定的に言えなくなるかのどちらかに倒れる。どちらも準承認の意図から外れる。1:1 という比率自体は導出値ではなく選択であり、27.5 の切替試験で見直す。

絶対上限を別に置くのは、相対上限が Layer 1 の件数に比例して緩むためである。逆に絶対上限だけでは、Layer 1 が 1 件しか当たらなかった Query で Layer 2 が支配的になる場合を防げない。二つは互いの死角を埋める。

守っている場所が違うこと:

本節の上限は**1回の Retrieval で Agent が見るもの**を制限する。未審査の Proposal が Scope 全体でどれだけ溜まっているかは制限しない。母集団の側は 27.3 の未審査比率が見る。

採らない形:

上限を超えた Layer 2 を、本文を落として title だけ渡す形にはしない。それは Layer 3 と同じ渡し方であり、「保留中の知識は使ってよいが、退役した知識は使ってはならない」という層の非対称を Agent 側から見分けられなくする。溢れたものは落とす。Review Queue には残るため、失われるのは当該 Retrieval での可視性だけである。
...
未審査比率は **Retrieval が返した結果ではなく、上限をかける前の母集団**で測る。21.1 節の相対上限により、返した結果の未審査比率は構造的に 50% を超えない。返した側で測れば、測っているのは上限であって系ではなくなる。
...
処理時間の閾値は運用の調整点だが、**未審査比率は準承認という設計そのものの反証条件である。**

比率が恒常的に 50% を超えるなら、Review が遅れても最終的には追いつくという 21.1 節の前提が成り立っていない。未審査のまま本文が使われる状態が定常になり、`unreviewed` タグは全件に付くことで情報を失う。そのとき採るべきは負荷対策ではなく、準承認の撤回を含む再設計である。
...
順序が重要である。出力を見てから正解を決めると、判定が出力に引きずられる。
