...
- v0.16: 読む側の保証を、セッションではなく行為に置く(30.1節)。行為の門を実装し、採用済みの知識を行為の直前に突きつけて一度拒否する。計器の嘘を直す。開始時の計数が JSON の足場を数えず、上限が実費の半分に対して掛かっていた(21.2節)。Layer 1 に token 上限を入れる。件数だけが上限で本文長が無制限だった(21.1節)。日次予算をすべての抽出経路に掛ける(16.3節)。完了した版を抽出の入力集合から外す(16.1節)。捕捉の走査を固定窓から watermark へ替え、走査の沈黙を health に載せる(16.3節)。Review 随意と矛盾していた待ち行列の警告を直す(17節)。検査ファイルの欠落が黙って通る経路を塞ぐ(24節)。事故分類に `source_preemption` を足す(27.5節)。Scope の現在状態の書き手を抽出へ置く(14節)
