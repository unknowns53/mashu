...
- **Scratch-first**: Scratch を書いたセッションは、Scratch の各項目が指す**時刻の周辺 turn だけ**を読む(前 6 / 後 2。note は対象の後に書かれるので後ろより前を厚く取る)。Scratch 項目の `created_at` と transcript の record 時刻を突き合わせて位置を決める。**時刻が決めるのは anchor の位置だけで、範囲は元の turn 列の上で切る。** 全 turn が無時刻なら全文へ落ちるが、危険なのは「一部だけ時刻がある」場合で、時刻のある turn の上で範囲を切ると anchor 1 本だけを読んで夜を捨てることになる。**Scratch が空のセッションは従来どおり全文を窓分割で読む。** 壊れ方をこの形にするのは、遵守したセッションが桁で安くなり、遵守しなかったセッションが今日と同じで、どちらも黙って悪化しないため。実測で 1 万 turn のセッションが、Scratch 20 件で 34 万 token / 6 窓から 2.3 万 token / 1 窓になる(14 倍)。対話寄りのセッションは turn 1 本が長いので 4 倍程度に留まる。抜粋の中では畳み込みを効かせないので、畳んだ場合の見かけの安さ(1.3 万)よりは高い。その差が正しさの代金
  - **明示の marker(「覚えて」等)は単独では発火条件にしない。** 一部だけ読むのが正当化できるのは「セッション自身が指した」からであって、何も指さなかったセッションは指していない。Scratch を書いたセッションの中でだけ、marker の周辺も読む対象に加える
  - **破棄するのは、その呼び出しが実際に読んだ項目だけ。** model 呼び出しは二つのトランザクションの間で走る(呼び出しを跨いでトランザクションを開いたままにすると、まさに Scratch を書いているセッションを塞ぐため)。だから考えている最中に置かれた項目は snapshot の後に届いていて、誰にも読まれていない。行ごと空にすると、それを未読のまま捨てることになる——Scratch が防ぐはずのことそのもの
  - 代償は隠さない。周辺に入らなかった turn は読まれず、checkpoint はその先へ進むので、一晩の作業が 2 件の Scratch だけで代表されうる。**監査行は run ごとではなく model 呼び出しごとに書く。** run は複数回の呼び出しで、最後に一度だけ書く形は最終窓の数字をセッションの数字として報告する。閾値を置かず記録から判断する方針は、記録が正しいことに全体重を預けている。残すのは、どちらの読みか / 送った turn 数 / 対象だった turn 数 / **二度と戻らない turn 数** / 時刻を持つ turn 数 / Scratch の件数と解決方式。「二度と戻らない」が非ゼロになるのは選択読みの最終 pass だけで、まだ続く窓は取らなかった分を捨ててはいない
...
- **同一 turn の畳み込み**: 一字一句同じ turn が再び現れたら、位置と role と先頭 12 字だけを残した place-holder に畳む。実測で、clip を通した後になお 33%(対話寄り)〜56%(Agent 主導)が既出の turn そのものだった。**畳み込むのは「既にモデルへ送った turn」に対してであって、「ファイル上で先に出てくる turn」に対してではない。** 先頭から取る窓どうしなら畳んでよい——窓 1 は窓 2 が指す本文を実際に運んでいる。ここに削減が住んでいて、窓の内側だけで畳むと 3%、窓を跨いで畳むと 50% になる(繰り返しが夜全体に散っていて固まっていないため)。**飛び飛びに選ぶ読みでは畳めない。** 初出が「どの呼び出しも開かなかった区間」にありうるので、誰も読んでいない本文への参照だけがモデルに渡り、しかも checkpoint はその先へ進む。実測で 1 セッション全体が 24 万→17 万(窓 4→3)、68 万→34 万(窓 11→6)
- **不変部分を先に置く**: Prompt は「本文 + Active 集合」「セッションログ」の順に組む。Caching は prefix しか再利用しないので、毎回変わるログを先頭に置くと後ろの不変部分まで窓ごとに払い直しになる。API 経路ではその境界に cache breakpoint を打つ。**ただし日次予算は cache hit も読んだ token として数える。** 予算が縛っているのは読む量であって金額なので、hit を除いて数えると上限が誰の判断も経ずに緩む
  - **Scratch の持ち主は ID では引けない。** MCP server は自分の CLI セッション ID を起動時の環境から読むが、会話が compaction / resume されると CLI は新しい ID を発行し、server は再起動されない。以後 Scratch は古い ID の行に書かれ、抽出される transcript は新しい ID を名乗る。transcript 側に親への参照は無く、名前では繋がらない(実測: 生存中の server 4 つが `e73a85f5` `0ac4b8f6` `930b105f` `65eecb5a` を保持している一方、実際に応答していた会話は `d940f18b` で、その行は存在しなかった)。**cwd と時刻で引く。** セッションは或るディレクトリに居て、Scratch の各項目は書かれた時刻を持ち、transcript は同じディレクトリと或る時間帯を名乗る。ID と違ってこれは CLI に書き換えられない。残る曖昧さは「同一ディレクトリで同時に 2 セッション」で、互いの note を拾う。代償は数 turn 余計に読むことと prompt に 1 行混じることで、**compaction を経た全セッションを黙って取り落とす**のとは釣り合わない。**曖昧なら partial-read を発火させず全文へ落とす。** 同じ cwd・同じ時間帯に候補が 2 つ以上あれば誰の note か決められない。全文を読むのは費用の面でだけ間違いだが、他人の flag で一部だけ読むのは**何を残すかの面で**間違いで、しかもこのセッションの残りは checkpoint の後ろへ永久に消える。ID で引いた行が存在して Scratch が空なら、それは「server がこの ID で居て何も書かなかった」なので場所へは落ちない(cwd を持つのは server が開いた行だけで、worker が自分の帳簿用に作る行は持たない。これが両者を分ける)。cwd は symlink と末尾 slash を正規化して比べる
- 日次の入力予算を持ち、超過は捨てずに**次の予算リセットまで** defer する。24 時間固定にしない。予算は真夜中に空くのに run は同じ時刻まで塞がったままで、その間に走る唯一の夜間巡回が閉じた run を見て、丸 1 日余計に待つことになる。60 件が defer された状態はそれでは減らない。**日次の集計は run の累計ではなく、model 呼び出しごとの行から取る。** `extraction_run.input_tokens` は run 全体の累計なので、真夜中を跨いで窓分割される run は前日までの支出を毎朝もう一度今日の分として数えられ、既に大半を使った run は二度と次の窓を買えないまま、予算は満杯だと報告しながら永久に defer し続ける
...
Scope はラベルではなく**その Scope が保持している Memory** から判定する。クエリを Memory 本文の索引——Layer 1 が既に十分な精度で順位づけしている、まさにその索引——に当て、上位の一致がどの Scope に集中しているかを見る。**絶対類似度の問いを順位と集中の問いに変える**わけで、実測が示したのは、この埋め込みが答えられるのは後者だけだということだった。

probe は全 Scope 横断の上位 25 件、floor 0.82。集中は**素の割合ではなく lift** で測る——「その Scope が上位一致の何割を占めるか」÷「その Scope が在庫の何割を占めるか」——が 1.2 以上、かつ最低 2 件で検出とする。

素の割合には**大きさへの不変性が無い**。しかも自動捕捉が入るとそれは理論上の話ではなくなる。route は実際に作業するディレクトリを指すので、ひとつの Scope が在庫の大半を持つようになり、以後は質量だけで全クエリを取る。比にすると問いが変わる——**この Scope は、その大きさから予測されるより良く答えているか**。極端側の壊れ方も正しい。在庫のほとんどを持つ Scope はまず比を超えられないので検出されないが、在庫の大半に等しい Scope へ絞り込んでも得るものは無い。

| クエリ | 旧(ラベル) | 新(保持 Memory) |
|---|---|---|
| 測定を始める前に何を決めておくべきか | 誤(enrai を上位) | 作業の規律 |
| サブエージェントに仕事を任せるときの作法 | 誤(enrai を上位) | 作業の規律 |
| 空母戦のレバーはどの probe で測るのが正しいか | enrai | enrai |
| 難易度を上げるときに敵をどう作るべきか | enrai | enrai |
| 未審査の知識をエージェントにどう渡すか | mashu | 検出なし |
| 今日の天気はどうか(無関係) | 誤(enrai を上位) | 検出なし |
| PNIPAM の曇点測定の昇温速度(該当 Scope なし) | 誤(mashu を上位) | 検出なし |

**誤った Scope への絞り込みが一度も起きない。** これが答えを丸ごと隠す唯一の失敗で、旧方式は 7 問中 4 問でそれをしていた。検出なしの 3 問は呼び出し側が「全体を検索する」と読むので、絞り込みが無いだけである。

floor は仕事をしている。0.78 まで下げると、天気のクエリが上位 10 件のうち 8 件を一つの Scope に x1.49 で集中させて誤検出する。0.84 まで上げると 7 問中 6 問が検出なしになる。

辞退する 3 問のうち 2 問は、辞退が正しい。「未審査の知識を〜」の mashu Scope は 2 件しか保持しておらず、**ほとんど何も持っていない Scope は、どれだけよく一致しても集中を示せない**。PNIPAM は在庫にその主題が無く、上位一致が x0.99 / x1.08 に割れる——これが「どの Scope も大きさ以上には答えていない」の見た目である。素の割合で測っていた版は、この問いを 2 Scope に絞り込んでいた。
