...
- v0.14: Review の扱いを、単位ではなく操作の側から直す。束を題名の目次として開き、矢印で読み、キー 1 打で決める形にする。決定はその場で 1 件ずつ確定し、中断しても手前が残る(18.1節)。--all が束そのものを広げていなかった経路を直す(18.1節)。待っている束の一覧を最上位に置き、画面に収まる分だけ描く
