...
probe は全 Scope 横断の上位 25 件、floor 0.82、同一 Scope が上位の 34% 以上(最低 2 件)を占めたときに検出とする。
...
| PNIPAM の曇点測定の昇温速度(該当 Scope なし) | 誤(mashu を上位) | 2 Scope |

**単一の誤った Scope への絞り込みが一度も起きない。** これが答えを丸ごと隠す唯一の失敗で、旧方式は 7 問中 4 問でそれをしていた。検出なしの 2 問は呼び出し側が「全体を検索する」と読むので、絞り込みが無いだけである。

floor は仕事をしている。0.78 まで下げると、天気のクエリが上位 10 件のうち 8 件を一つの Scope に集中させて誤検出する。0.84 まで上げると 7 問中 6 問が検出なしになる。

「未審査の知識を〜」が検出なしなのは、mashu Scope が 2 件しか保持していないためである。**ほとんど何も持っていない Scope は集中を示せない。** これは機構の失敗ではなく辞退であり、そのクエリはもともと全体検索へ行くべきだった。
