...
- **Scratch-first**: Scratch を書いたセッションは、Scratch の各項目が指す**時刻の周辺 turn だけ**を読む(前 6 / 後 2。note は対象の後に書かれるので後ろより前を厚く取る)。Scratch 項目の `created_at` と transcript の record 時刻を突き合わせて位置を決める。**Scratch が空のセッションは従来どおり全文を窓分割で読む。** 壊れ方をこの形にするのは、遵守したセッションが桁で安くなり、遵守しなかったセッションが今日と同じで、どちらも黙って悪化しないため。実測で 1 万 turn のセッションが、Scratch 20 件で 36 万 token / 7 窓から 1.3 万 token / 1 窓になる(28 倍)。対話寄りのセッションは turn 1 本が長いので 4 倍程度に留まる
  ...
  - 代償は隠さない。周辺に入らなかった turn は読まれず、checkpoint はその先へ進むので、一晩の作業が 2 件の Scratch だけで代表されうる。**どちらの読みが走ったか**と**何 turn 中何 turn を読んだか**を `extraction_filed` の detail に記録する。一部読みが recall を損なうかは 27.4 で問う話で、どちらだったか言わない run には問えない
