...
行為は判断の名前であって、ツールの名前ではない。ひとつの判断しか表さないツールは名前で引く（`ACTIONS`。`Task` / `Agent` / `codex_start` はどれも `delegate`）。複数の判断が同じツールを通るなら、渡されたコマンドのほうを読む（`BY_COMMAND`）。どの形にも当たらないコマンドは行為を持たず、門は立たない。

コマンドと行為の対応表はリポジトリの外に置く。禁止パターンの一覧と同じ置き方で、理由は裏返しである。あちらは公開してはいけないものを持ち、こちらはひとつのインストールしか知らないものを持つ。クライアントがどのツールを備えるかはそのクライアントが動くところなら同じなので `ACTIONS` は同梱するが、どのコマンドがどのクラスタ・ラッパー・スケジューラに届くかは一つのサイトでしか正しくない。既定は `.mashu-guard-actions`（gitignore 済み）で、`MASHU_GUARD_ACTIONS` で場所を変えられる。

```
# 1 行 1 規則。行為が先、残りがコマンドの正規表現。先に当たったものが勝つ
remote-shell \b(cluster-wrapper|job-scheduler-cmd)\b
```

この表が無いとき、あるいは空のときは、どのコマンドも行為を持たない。門は `ACTIONS` が置く場所にだけ立つ、つまりこの表ができる前と同じ振る舞いに戻る。compile できない行は読み飛ばして残りを生かす。

コマンドを読む側に降りたのは、ツール名で足りると考えた結果が実際に壊れたからである。`Bash` を一律ひとつの行為に写したところ、遠隔クラスタについての記憶 5 件がセッション最初の `ls` の前に出た。ツールの見立ては合っていて判断の見立てが外れており、出てきたものは正しいのに読まれない。それは 16.3 が名指しする「誰も読まない門」そのものである。

文字列で見ている以上、コマンドがその語を実行ではなく引用として含むときも門は立つ。読んでもう一度呼べば通るので害は小さいが、当たらないより当たりすぎるほうを選んでいる。
