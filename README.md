...
| `mashu stale` | 寿命を持ったまま立っている知識を順に見て、キー 1 打で退役させる |
...
どの画面も端末の高さに収まる分だけ出す。入りきらない分は `↓ N more` のように件数で言う。横幅も端末に合わせて折る。
...
`mashu stale` はそれを順に見せる。三つの読みで拾う。
...
`s` は「いつまた聞くか」を尋ねる（空欄で 30 日）。その間は一覧に出ない。**記録するのは知識ではなく読んだ側の事実**——ある日に人が読んで、退役させなかった——なので、version も status も動かない。event_log に `still_stands` として残る。
