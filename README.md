...
行為は判断の名前であって、ツールの名前ではない。ひとつの判断しか表さないツールは名前で引く（`ACTIONS`。`Task` / `Agent` / `codex_start` はどれも `delegate`）。複数の判断が同じツールを通るなら、渡されたコマンドのほうを読む（`BY_COMMAND` と `COMMAND_ACTIONS`。`Bash` のうち `rccs-run` や `waitest` を含むものだけが `remote-shell`）。どの形にも当たらないコマンドは行為を持たず、門は立たない。
