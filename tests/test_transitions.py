...
@pytest.mark.parametrize("target", [S.SUPERSEDED, S.DISPROVEN, S.DORMANT, S.REJECTED, S.COMPLETED])
def test_a_candidate_can_reach_every_outcome(target):
    check_transition(S.CANDIDATE, target)


@pytest.mark.parametrize("status", [S.SUPERSEDED, S.DISPROVEN, S.DORMANT, S.REJECTED])
...
def test_only_a_candidate_can_be_rejected():
    """Rejection is a verdict on a proposal, and only a candidate is awaiting one."""
    with pytest.raises(TransitionError, match="cannot move completed to rejected"):
        check_transition(S.COMPLETED, S.REJECTED)
